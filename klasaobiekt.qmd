...
8.  Stwórz tablicę obiektów klasy `Car` (klasa zawiera pole `brand`). Następnie spróbuj odwołać się do pola `brand` jednego z obiektów w tablicy, nie inicjalizując wcześniej tablicy obiektami `Car`. Przeanalizuj wynik.

9.  Stwórz listę tablicową (ArrayList) obiektów klasy `Person` (klasa zawiera pole `name`). Następnie spróbuj odwołać się do pola `name` jednego z obiektów na liście, nie dodając wcześniej do listy żadnych obiektów `Person`. Przeanalizuj wynik.
